...
子ども向けの静的 Web アプリです。日常の場面をもとに、「次にどうするとよいか」「だれに伝えるべきか」「相手は何を知らないか」を考える 20 問を収録し、毎回その中から 10 問をランダム出題します。
...
- 20 問をデータファイルとして分離し、毎回 10 問をランダム出題
- カテゴリ別スコア表示あり
- タイトル画面と問題画面で使える SVG イラスト同梱
...
4. `illustration` に `src/assets/illustrations/` の SVG を指定する
